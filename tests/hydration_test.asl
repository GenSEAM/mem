...
(df test-raw-context-eviction [] -> Bool
  ...
  (let [(c1 (vl/make-memory-chunk "c-pre-1" 1000 0.90 "pre-existing" (list) (list) (list)))
...
(df test-markdown-hydration-and-card [] -> Bool
  ...
  (let [(c (vl/make-memory-chunk
        ...
        (c-empty (vl/make-memory-chunk "chunk-empty" 1000 0.90 "empty" (list) (list) (list)))
...
(df test-summary-search-and-autoloading [] -> Bool
  ...
  (let [(idx0 (ss/summary-index-empty))
        (c1 (vl/make-memory-chunk "c-s-1" 1000 0.95 "Memory persistence and cascade amnesia" (list) (list) (list)))
        (c2 (vl/make-memory-chunk "c-s-2" 2000 0.90 "Human hydration view layer projection" (list) (list) (list)))
...
(df test-hydrate-intent-summary [] -> Bool
  ...
  (let [(axioms (h/default-core-axioms))
        ...
        (syn (h/synthesize-intent-summary "(:id \"D0015\" :title \"Axiom of Token Arbitrage\")"))
        (syn-fail (h/synthesize-intent-summary "empty"))]
    (assert (= (list-length axioms) 4) "Default core axioms must contain exactly 4 entries")
    (assert (string-contains? summary "D0015: Token Arbitrage") "Summary must include Axiom 1 (D0015)")
    (assert (string-contains? summary "D0016: Agent-Native Autonomy") "Summary must include Axiom 2 (D0016)")
    (assert (string-contains? summary "D0017: Falsifiable Observability") "Summary must include Axiom 3 (D0017)")
    (assert (string-contains? summary "D0018: In-Memory State Surgery") "Summary must include Axiom 4 (D0018)")
    (assert (string-contains? empty-summary "Token Arbitrage") "Empty axioms list must fall back to canonical axiom names")
    (assert (string-contains? syn "D0015") "Synthesized summary must contain D0015 on matching ledger")
...
(df test-modern-intent-ledger-hydration [] -> Bool
  :d "Verifies deterministic hydration of modern phase decisions D0019 through D0029 and liquidation of legacy PCP hashes."
  (let [(d19 (h/hydrate-modern-intent "D0019"))
        (d20 (h/hydrate-modern-intent "D0020"))
        (d21 (h/hydrate-modern-intent "D0021"))
        (d22 (h/hydrate-modern-intent "D0022"))
        (d23 (h/hydrate-modern-intent "D0023"))
        (d24 (h/hydrate-modern-intent "D0024"))
        (d25 (h/hydrate-modern-intent "D0025"))
        (d26 (h/hydrate-modern-intent "D0026"))
        (d27 (h/hydrate-modern-intent "D0027"))
        (d28 (h/hydrate-modern-intent "D0028"))
        (d29 (h/hydrate-modern-intent "D0029"))
    ...
    (assert (string-starts-with? d19 "D0019") "D0019 must begin with identifier prefix")
    (assert (string-contains? d19 "Differential Test Runner") "D0019 must hydrate differential test runner decision")
    (assert (string-starts-with? d20 "D0020") "D0020 must begin with identifier prefix")
    (assert (string-contains? d20 "Token Regex Engine") "D0020 must hydrate token regex engine decision")
    (assert (string-starts-with? d21 "D0021") "D0021 must begin with identifier prefix")
    (assert (string-contains? d21 "Equivalence E-Graphs") "D0021 must hydrate AST equivalence e-graphs decision")
    (assert (string-starts-with? d22 "D0022") "D0022 must begin with identifier prefix")
    (assert (string-contains? d22 "Speculative VFS Branching") "D0022 must hydrate speculative VFS branching decision")
    (assert (string-starts-with? d23 "D0023") "D0023 must begin with identifier prefix")
    (assert (string-contains? d23 "Grammar-Trie Constrained") "D0023 must hydrate grammar-trie decoding decision")
    (assert (string-starts-with? d24 "D0024") "D0024 must begin with identifier prefix")
    (assert (string-contains? d24 "Memory TaskStore") "D0024 must hydrate Memory taskstore decision")
    (assert (string-starts-with? d25 "D0025") "D0025 must begin with identifier prefix")
    (assert (string-contains? d25 "Supervisor & Dual-Temperature") "D0025 must hydrate supervisor speculative racing decision")
    (assert (string-starts-with? d26 "D0026") "D0026 must begin with identifier prefix")
    (assert (string-contains? d26 "WASI Lowering") "D0026 must hydrate pure ASL WASI lowering decision")
    (assert (string-starts-with? d27 "D0027") "D0027 must begin with identifier prefix")
    (assert (string-contains? d27 "Dense Tabular Pyramid") "D0027 must hydrate compact tabular pyramid decision")
    (assert (string-starts-with? d28 "D0028") "D0028 must begin with identifier prefix")
    (assert (string-contains? d28 "Cross-Reference URIs") "D0028 must hydrate universal cross-reference URIs decision")
    (assert (string-starts-with? d29 "D0029") "D0029 must begin with identifier prefix")
    (assert (string-contains? d29 "Decoupled Amnesia") "D0029 must hydrate decoupled amnesia decision")
    ...
    (assert (not (string-empty? d19)) "Hydrated D0019 must not be empty")
    (assert (not (string-empty? d29)) "Hydrated D0029 must not be empty")
    true))
