...
(df test-chunk-validation [] -> Bool
  ...
  (let [(c-valid (a/make-memory-chunk "c-v" 1725769200000 0.85 "valid" (list) (list) (list)))
        (c-zero-ts (a/make-memory-chunk "c-zts" 0 0.85 "zero ts" (list) (list) (list)))
        (c-neg-conf (a/make-memory-chunk "c-nc" 1000 -0.1 "neg conf" (list) (list) (list)))
        (c-high-conf (a/make-memory-chunk "c-hc" 1000 1.2 "high conf" (list) (list) (list)))
        (c-low (a/make-memory-chunk "c-low" 1000 0.45 "low conf" (list) (list) (list)))
        (c-high (a/make-memory-chunk "c-high" 1000 0.92 "high conf" (list) (list) (list)))]
...
(df test-dag-construction [] -> Bool
  ...
  (let [(d0 (a/chunk-dag-empty))
        (r1 (a/make-memory-chunk "root-1" 1000 0.90 "root chunk" (list) (list) (list)))
    ...
    (let [(d1 (result-or res-add-r1 d0))
          (c1 (a/make-memory-chunk "child-1" 2000 0.88 "child chunk" (list) (list) (list "root-1")))
...
(df test-dag-ancestors [] -> Bool
  ...
  (let [(d0 (a/chunk-dag-empty))
        (na (a/make-memory-chunk "A" 1000 0.95 "Node A" (list) (list) (list)))
        (d1 (result-or (a/chunk-dag-add d0 na) d0))
        (nb (a/make-memory-chunk "B" 2000 0.95 "Node B" (list) (list) (list "A")))
        (d2 (result-or (a/chunk-dag-add d1 nb) d1))
        (nc (a/make-memory-chunk "C" 3000 0.95 "Node C" (list) (list) (list "B")))
    ...
    (let [(nc-dia (a/make-memory-chunk "C2" 2500 0.95 "Node C2" (list) (list) (list "A")))
          (d-dia1 (result-or (a/chunk-dag-add d2 nc-dia) d2))
          (nd (a/make-memory-chunk "D" 4000 0.95 "Node D" (list) (list) (list "B" "C2")))
...
(df test-cycle-prevention [] -> Bool
  ...
  (let [(d0 (a/chunk-dag-empty))
        (c-self (a/make-memory-chunk "A" 1000 0.90 "self loop" (list) (list) (list "A")))]
    ...
    (let [(na (a/make-memory-chunk "A" 1000 0.90 "Node A" (list) (list) (list)))
          (d1 (result-or (a/chunk-dag-add d0 na) d0))
          (nb (a/make-memory-chunk "B" 2000 0.90 "Node B" (list) (list) (list "A")))
          (d2 (result-or (a/chunk-dag-add d1 nb) d1))
          (c-2node (a/make-memory-chunk "A" 3000 0.90 "2-node loop" (list) (list) (list "B")))]
      (assert (a/detect-cycle? d2 c-2node) "Two-node cycle A -> B -> A must trigger cycle detection")
      (let [(nc (a/make-memory-chunk "C" 3000 0.90 "Node C" (list) (list) (list "B")))
            (d3 (result-or (a/chunk-dag-add d2 nc) d2))
            (c-indirect (a/make-memory-chunk "A" 4000 0.90 "indirect loop" (list) (list) (list "C")))
            (c-orphan (a/make-memory-chunk "O" 5000 0.90 "orphan" (list) (list) (list "unknown-parent")))]
